...
Note that with 'load.cpp' also you can execute experiments to measure the size/locate_time for the index, where the patterns will read from files.
The format of this file of patterns is one string for line, with all the patterns with the same length for each different file: 'patt_m' where ‘m’ is the length of each pattern in the file. The number of patterns also is given as parameter in the main function (see the documentation in these files for more details).
